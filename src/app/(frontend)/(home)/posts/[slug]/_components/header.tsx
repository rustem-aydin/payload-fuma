...
export const Header = ({ page, tags }: HeaderProps) => {
  ...
  const image = typeof page.meta?.image === 'object' 
    ? (page.meta.image as Media).url 
    : null

  const formattedDate = formatPostDate(new Date(page.publishedAt || page.createdAt))
          ...
          >
            ...
            <div className='mb-4 space-y-4 text-balance'>
              ...
              <p className='typography-body hidden max-w-3xl text-slate-100 leading-8 md:block'>
                <Balancer>{page.meta?.description ?? ''}</Balancer>
              </p>
            </div>
