...
// Payload tiplerini import ediyoruz
...
interface HeaderProps {
  page: Post; // MDXPage yerine Post
  tags?: string[];
}

export const Header = ({ page, tags }: HeaderProps) => {
  // Payload Resim yolu: page.meta.image.url
  // Payload Resim yolu: page.meta.image.url
  const metaImage =
  ...
  // 2. Eğer SEO resmi yoksa, Kapak Resmine (heroImage) bak
  const heroImage =
    page.heroImage && typeof page.heroImage === "object"
      ? (page.heroImage as Media).url
      : null;

  // 3. Hangisi varsa onu kullan (Meta öncelikli)
  const image = heroImage || metaImage;

  const formattedDate = formatPostDate(
    new Date(page.publishedAt || page.createdAt),
  );
    ...
    <Section>
      ...
      <div className="relative h-[350px] overflow-clip md:h-[600px]">
        <div className="relative flex h-full w-full flex-col justify-end overflow-hidden rounded-2xl bg-primary/80 shadow-xl">
          {image && (
            <BlurImage
              alt={page.title ?? "Blog cover image"}
          ...
          >
            ...
            <div className="mb-4 space-y-4 text-balance">
              <h1 className="typography-hero max-w-2xl font-medium text-4xl text-white leading-[45px] tracking-tight md:text-5xl md:leading-[60px]">
                <Balancer>{page.title ?? "Untitled"}</Balancer>
              </h1>
...
// FrameDecoration aynı kalabilir...
